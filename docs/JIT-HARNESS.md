...
This package expresses **GitHub Code Review** as a typed, task-adaptive agent harness. Its concrete capability remains: Review pull requests for correctness, security, tests, and maintainability. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in GitHub Code Review |
| --- | --- | --- |
| Memory | `before_after_evidence` | Retain pull request diff plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `prepare_execute_verify_plan` | Transform repository context into the ordered, bounded stages defined by this skill. |
...
| Action | `inspect_change_test_handoff` | Advance the workflow through risk analysis while preserving stop conditions and user authority. |
